...
void AKAZE::set_akaze_options(const AKAZEOptions& options){
	evolution_.clear();
	options_ = options;
...
void AKAZE::Feature_Detection(std::vector<cv::KeyPoint>& kpts) {
  ...
  double t11 = 0.0, t21 = 0.0, total = 0;
  ...
  t21 = cv::getTickCount();
  total = 1000.0*(t21-t11) / cv::getTickFrequency();
  cout << "Time Hessian response: " << total << endl;

  t11 = cv::getTickCount();
  Find_Scale_Space_Extrema(kpts);
  t21 = cv::getTickCount();
  total = 1000.0*(t21-t11) / cv::getTickFrequency();
  cout << "Time Scale Space Extrema: " << total << endl;

  t11 = cv::getTickCount();
  Do_Subpixel_Refinement(kpts);
  t21 = cv::getTickCount();
  total = 1000.0*(t21-t11) / cv::getTickFrequency();
  cout << "Time subpixel refinement: " << total << endl;


  t2 = cv::getTickCount();
  timing_.detector = 1000.0*(t2-t1) / cv::getTickFrequency();

  cout <<  "Feature detection: " << timing_.detector << endl;
}
...
/* ************************************************************************* */
std::vector<cv::Mat> AKAZE::Get_Scale_Space() {

  cv::Mat img_aux;
  string outputFile;
  
  std::vector<cv::Mat> pyramid;
  pyramid.resize(evolution_.size());
